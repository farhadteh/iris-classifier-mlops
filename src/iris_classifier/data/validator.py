...
class DataValidator:
    ...
    def validate_prediction_input(
        ...
        for feature in self.expected_features:
            ...
            if value < min_val or value > max_val:
                errors.append(
                    f"Feature '{feature}' value {value} is outside valid range [{min_val}, {max_val}]"
                )
    ...
    def validate_batch_input(
        self, batch_data: List[Dict[str, float]]
    ...
    def validate_full_dataset(
        self, data: pd.DataFrame, targets: Optional[pd.Series] = None
    ) -> Dict[str, Any]:
        """
        ...
        logger.info(
            f"Dataset validation completed. Valid: {validation_results['is_valid']}"
        )
