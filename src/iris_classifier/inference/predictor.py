...
class ModelPredictor:
    ...
    def predict(self, input_data: Dict[str, float], return_probabilities: bool = True) -> Dict[str, Any]:
        """
    ...
    def predict_with_uncertainty(self, input_data: Dict[str, float], 
        ...
        """
        logger.debug(f"Making prediction with uncertainty estimation ({n_samples} samples)")
    ...
    def explain_prediction(self, input_data: Dict[str, float]) -> Dict[str, Any]:
        ...
        for feature, value in input_data.items():
            if feature in feature_stats:
                ...
                normalized_value = (value - stats['mean']) / (stats['max'] - stats['min'])
                explanation['feature_analysis'][feature] = {
                    'value': value,
                    'normalized': float(normalized_value),
                    'percentile': float((value - stats['min']) / (stats['max'] - stats['min']) * 100)
                }
