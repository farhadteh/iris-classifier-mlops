...
page = st.sidebar.selectbox("Choose a page", ["Single Prediction", "Batch Prediction", "Model Explorer", "API Status"])
...
if page == "Single Prediction":
    ...
    fig_features = px.bar(feature_data, x='Feature', y='Value', 
    ...
    if st.button("🔮 Predict", type="primary"):
        ...
        if success:
            ...
            prob_data = pd.DataFrame(list(result['probabilities'].items()), 
                                   columns=['Class', 'Probability'])
            
            fig_prob = px.bar(prob_data, x='Class', y='Probability',
    ...
    if uploaded_file is not None:
        try:
            ...
            if all(col in df.columns for col in required_cols):
                if st.button("🔮 Predict Batch", type="primary"):
                    ...
                    if success:
                        st.success(f"✅ Batch prediction successful! {result['batch_size']} predictions made.")
                        ...
                        result_df = pd.concat([df[required_cols], predictions_df], axis=1)
                        ...
                        with col1:
                            ...
                            fig_dist = px.pie(values=class_counts.values, names=class_counts.index,
                                            title='Predicted Class Distribution')
                            st.plotly_chart(fig_dist, use_container_width=True)
                        
                        with col2:
                            # Confidence distribution
                            fig_conf = px.histogram(predictions_df, x='confidence',
        ...
        if st.session_state.batch_data:
            ...
            with col1:
                if st.button("🔮 Predict Batch", type="primary"):
                    with st.spinner(f"Making {len(st.session_state.batch_data)} predictions..."):
                        success, result = make_batch_prediction(st.session_state.batch_data)
        ...
        if success:
            ...
            for i, feature in enumerate(features):
                ...
                for j, class_name in enumerate(['setosa', 'versicolor', 'virginica']):
                    class_data = full_df[full_df['predicted_class'] == class_name][feature]
                    fig_dist.add_trace(
                        go.Histogram(x=class_data, name=f'{class_name}', 
                                   marker_color=colors[j], opacity=0.7),
                        row=row, col=col
                    )
            
            fig_dist.update_layout(title='Feature Distributions by Predicted Class', height=600)
            st.plotly_chart(fig_dist, use_container_width=True)
    ...
    while True:
        with status_placeholder.container():
            ...
            if healthy:
                ...
                with col2:
                    st.metric("Model Status", health_data.get('model_status', 'Unknown'))
                ...
                if success:
                    ...
                    with info_col1:
                        ...
                        st.metric("Model Version", model_data.get('model_version', 'Unknown'))
                    with info_col2:
                        st.metric("Model Stage", model_data.get('model_stage', 'Unknown'))
                        st.metric("Run ID", model_data.get('mlflow_run_id', 'Unknown')[:8] + "...")
