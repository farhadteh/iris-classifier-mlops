...
class FeatureSelector:
    ...
    def select_by_model_importance(
        self, X: pd.DataFrame, y: pd.Series, estimator=None, threshold: str = "median"
    ) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        """
        logger.info(
            f"Selecting features using model importance (threshold: {threshold})"
        )
        ...
        logger.info(
            f"Selected {len(selected_features)} features using model importance"
        )
        return (
            pd.DataFrame(X_selected, columns=selected_features, index=X.index),
            selected_features,
    ...
    def recursive_feature_elimination(
        self, X: pd.DataFrame, y: pd.Series, n_features: int = 10, estimator=None
    ) -> Tuple[pd.DataFrame, List[str]]:
        """
        ...
        """
        logger.info(
            f"Performing recursive feature elimination to select {n_features} features"
        )
        ...
        self.feature_scores = {
            k: 1.0 / v
            for k, v in sorted(ranking_scores.items(), key=lambda item: item[1])
        }
    ...
    def select_by_correlation_threshold(
    ...
    def select_by_variance_threshold(
    ...
    def comprehensive_feature_selection(
        self, X: pd.DataFrame, y: pd.Series, max_features: int = 10
        ...
        # Statistical test
        try:
            _, features_stat = self.select_k_best_features(
                X_corr, y, k=min(max_features, len(X_corr.columns))
            )
            methods_scores["statistical"] = {
                f: self.feature_scores.get(f, 0) for f in features_stat
            }
        ...
        try:
            _, features_model = self.select_by_model_importance(
                X_corr, y, threshold="median"
            )
            methods_scores["model_based"] = {
                f: self.feature_scores.get(f, 0) for f in features_model
            }
        except Exception as e:
        ...
        logger.info(
            f"Comprehensive selection completed. Selected {len(selected_features)} features"
        )
    ...
    def get_feature_selection_report(self) -> Dict[str, any]:
        ...
        return {
            ...
            "top_5_features": (
                dict(list(self.feature_scores.items())[:5])
