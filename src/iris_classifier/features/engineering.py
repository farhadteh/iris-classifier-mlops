...
class FeatureEngineer:
    ...
    def create_ratio_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_new["sepal_area"] = (
            np.pi * (df["sepal_length"] / 2) * (df["sepal_width"] / 2)
        )
        df_new["petal_area"] = (
            np.pi * (df["petal_length"] / 2) * (df["petal_width"] / 2)
        )
    ...
    def create_polynomial_features(
        ...
        logger.info(
            f"Created polynomial features for {len(self.feature_names)} base features"
        )
        return df_new

    def create_all_features(
        self, df: pd.DataFrame, include_polynomial: bool = False
    ) -> pd.DataFrame:
        """
        ...
        logger.info(
            f"Total engineered features created: {len(self.engineered_features)}"
        )
