...
class IrisModelTrainer:
    ...
    def __init__(self, mlflow_uri: str = "file:./mlruns", experiment_name: str = "Iris Classification"):
    ...
    def calculate_metrics(self, y_true: np.ndarray, y_pred: np.ndarray, prefix: str) -> Dict[str, float]:
    ...
    def log_plots(self, model, X_train: np.ndarray, y_train: np.ndarray, 
                 X_test: np.ndarray, y_test: np.ndarray, feature_names: list):
    ...
    def run_experiment(self, models_to_train: list = None, tune_hyperparameters: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"Best model: {self.best_model['name']} (accuracy: {self.best_model['accuracy']:.4f})")
    ...
    def log_model_comparison(self, results: Dict[str, Any]):
        ...
        logger.info("Model comparison saved to model_comparison.png and model_comparison.csv")
...
def main():
    ...
    parser.add_argument("--experiment", default="Iris Classification",
