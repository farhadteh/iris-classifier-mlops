...
class RandomForestModel(BaseModel):
    ...
    def get_tree_count(self) -> int:
        ...
        return (
            ...
            if self.is_fitted
            else self.parameters.get("n_estimators", 0)
        )
...
class HyperparameterTunedModel:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized hyperparameter tuning for {base_model_class.__name__}"
        )
...
def create_model(model_type: str, **kwargs) -> BaseModel:
    ...
    if model_type not in model_classes:
        raise ValueError(
            f"Unknown model type: {model_type}. Available: {list(model_classes.keys())}"
        )
