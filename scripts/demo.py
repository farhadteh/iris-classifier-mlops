...
def run_training_demo():
    ...
    try:
        ...
        result = subprocess.run(
            ["python", "train.py", "--models", "logistic_regression", "random_forest"],
            capture_output=True,
            text=True,
            timeout=300,
        )
...
def test_api_endpoints():
    ...
    if not check_service(f"{api_base}/health", "FastAPI"):
        print(
            "Please start the FastAPI server first: uvicorn fastapi_app:app --host 0.0.0.0 --port 8000"
        )
        return False
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"   Prediction: {result['class_name']} (confidence: {result['confidence']:.3f})"
            )
        else:
            print(
                f"❌ Single prediction failed: {response.status_code} - {response.text}"
            )
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for i, pred in enumerate(result["predictions"]):
                print(
                    f"   Sample {i+1}: {pred['class_name']} (conf: {pred['confidence']:.3f})"
                )
        else:
            print(
